...
**PHASE 6 COMPLETE (2026-08-30):** final report written into CHANGELOG (Final row) and README (comparison table). Main contribution identified: retrieval + verifiable citations inverts pushback behavior.
**PHASE 6a build #1 COMPLETE (2026-08-30):** polar domain — 1,341 chunks / 3 sources, zero parser changes, zero errors, 1 gate, 90 min (embedding-rate-bound); smoke test 5/5 with citations (`eval/runs/smoke/polar-2026-08-30/`). Build #2 (aviation + CSV → tabular pipeline) OPEN — owner decision pending.
**PHASE 7 in progress:** README.md, REPRODUCE.md, SKILL.md, trajectories/ committed; video script drafted (`docs/VIDEO_SCRIPT.md`) — recording is the owner's step. Remaining: owner records video; optional aviation build.
**PHASE 5 COMPLETE (2026-08-30):** agent-side instruction ships in the MCP server's `instructions` string (every host inherits it). Verified live through real MCP plumbing: a fresh judgment question (slavery under early British administration) triggered 3 distinct decomposed search_corpus calls and produced a fully cited four-source synthesis — captured as `trajectories/phase5-agentic-decomposition.md`.
**PHASE 4 COMPLETE (2026-08-30):** 4a hybrid KEPT (rank-probe MRR 0.900 vs 0.733; server default RAG_MODE=hybrid; answer-level re-run held 26/26 with zero regressions — `eval/runs/solution/iter2-hybrid-2026-08-30/`). 4b query rewriting REMOVED (MRR 0.783 < 0.900; recorded as the removed experiment, CHANGELOG Iteration 3). Known limitation: Q7's exact GT chunk evades top-8 in all configs (fact still answered via a duplicate passage).
